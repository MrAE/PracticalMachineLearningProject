...
```{r model, eval=FALSE}
i <- 33; j <- i+3
featurePlot(x = training1[,i:j], y=training1$classe, plot="pairs")
modFitGLM   <- train(classe ~ ., method="glm", data=train1)
modFitRpart <- train(classe ~ ., method="rpart", data=train1)
modFitRF <- train(classe ~ ., method="rf", data=train1[sample(19000,1000),], prox=TRUE)
print(modFit$finalModel)
```

```{r pred}
modPred <- predict(modFit, newdata=test1)
